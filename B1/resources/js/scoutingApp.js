...
function resetRobot() {
  document.querySelector('input[value="b1"]').checked = true;
}
...
function updateMatchStart(event) {
  ...
  // Auto-fill team number for B1
  if (event.target.id == "input_m") {
    const match = getCurrentMatch();
    if (match && match.blue && match.blue.team_keys) {
      const b1TeamNumber = match.blue.team_keys[0].replace("frc", "");
      document.getElementById("input_t").value = b1TeamNumber;
      onTeamnameChange();
    }
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  // Set initial robot selection to B1
  document.querySelector('input[value="b1"]').checked = true;
