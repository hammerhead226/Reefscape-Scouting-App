...
function addTeamReminder(table, idx) {
  var row = table.insertRow(idx);
  var cell = row.insertCell(0);
  cell.setAttribute("colspan", "2");
  cell.setAttribute("style", "text-align: center; font-weight: bold;");
  cell.innerHTML = '<div id="teamReminder" style="color: #00ffff;">Scouter: <span id="currentScouter"></span><br>Currently Scouting: Team <span id="currentTeam"></span></div>';
  return idx + 1;
}
...
function configure() {
  ...
  idx = 0;
  idx = addTeamReminder(at, idx);  // Add team reminder at top
  ac.forEach(element => {
  ...
  idx = 0;
  idx = addTeamReminder(tt, idx);  // Add team reminder at top
  tc.forEach(element => {
  ...
  idx = 0;
  idx = addTeamReminder(egt, idx);  // Add team reminder at top
  egc.forEach(element => {
...
function resetRobot() {
  document.querySelector('input[value="b3"]').checked = true;
}
...
function onFieldClick(event) {
  ...
  /**
   * Handles click interactions for interactive elements on the scouting interface
   * @param {string} toggleClick - Flag indicating if toggle clicking is enabled ('true'/'false')
   * @param {Array} boxArr - Array storing box identifiers
   * @param {Array} xyArr - Array storing coordinate pairs
   * @param {string} box - Current box identifier
   * @param {Object} coords - Coordinate object {x, y}
   * @param {HTMLElement} changingInput - Input element storing box data
   * @param {HTMLElement} changingXY - Input element storing coordinate data
   * @param {string} clickRestriction - Type of click restriction ('one', 'onePerBox', or none)
   * @param {Object} cycleTimer - Timer object for cycle tracking
   */

  // Toggle click removal logic
  if ((toggleClick.toLowerCase() == 'true') && (boxArr.includes(box))) {
    // Remove item if toggle clicking is enabled and box exists
    let idx = boxArr.indexOf(box);
    boxArr.splice(idx, 1);     // Remove box from array
    xyArr.splice(idx, 1);      // Remove corresponding coordinates

    // Update input values with modified arrays
    changingInput.value = JSON.stringify(boxArr);
    changingXY.value = JSON.stringify(xyArr);
  } else {
    // Addition logic with different restrictions

    /**
     * Case 1: First item being added
     * Checks if the coordinate value array is empty (length <= 2 accounts for '[]')
     */
    ...
    /**
     * Case 2: Single item restriction
     * Only one item allowed - replaces existing item
     */
    else if (clickRestriction == "one") {
      // Replace existing box and coordinates
      changingXY.value = JSON.stringify([coords]);
      changingInput.value = JSON.stringify([box]);
    }
    /**
     * Case 3: One per box restriction
     * Allows multiple items but only one per unique box
     */
    else if (clickRestriction == "onePerBox") {
      // Add only if box isn't already in the array
      if (!boxArr.includes(box)) {
        boxArr.push(box);
        changingInput.value = JSON.stringify(boxArr);

        // Calculate and store coordinates for the box center
     ...
     * Allows multiple items and multiple per box
     */
    else {
      // Add new coordinates and box to arrays
      xyArr.push(coords);
      changingXY.value = JSON.stringify(xyArr);
    ...
    /**
     * Cycle Timer Integration
     * If associated with a cycle timer, trigger the cycle click event
     */
    if (cycleTimer != null) {
   ...
   * boxArr: ['box1', 'box2', ...]
   * xyArr: [{x: 100, y: 200}, {x: 150, y: 250}, ...]
   * changingInput.value: JSON string of boxArr
   * changingXY.value: JSON string of xyArr
   * 
   * Dependencies:
   * - findMiddleOfBox() function for coordinate calculation
   * - DOM elements with 'cycle_' + timer value IDs for cycle timing
   * 
   * Side Effects:
   * - Modifies boxArr and xyArr arrays
   * - Updates changingInput and changingXY input values
   * - May trigger cycle timer clicks
   */
...
function updateRobotPositions() {
  ...
  if (!match || !match.red || !match.blue) {
    return;
  }

  // Update Red Teams
  for (let i = 0; i < 3; i++) {
    const teamSpan = document.getElementById(`r${i+1}_team`);
    if (teamSpan) {
      const teamNumber = match.red.team_keys[i].replace('frc', '');
      teamSpan.textContent = teamNumber;
    }
  }

  // Update Blue Teams
  for (let i = 0; i < 3; i++) {
    const teamSpan = document.getElementById(`b${i+1}_team`);
    if (teamSpan) {
      const teamNumber = match.blue.team_keys[i].replace('frc', '');
      teamSpan.textContent = teamNumber;
    }
  }
}
...
function updateMatchStart(event) {
  ...
  // Auto-fill team number for B3
  if (event.target.id == "input_m") {
    const match = getCurrentMatch();
    if (match && match.blue && match.blue.team_keys) {
      const b3TeamNumber = match.blue.team_keys[2].replace("frc", "");  // Index 2 for B3
      document.getElementById("input_t").value = b3TeamNumber;
      onTeamnameChange();
    }
  }
}
...
function onTeamnameChange(event) {
  ...
  var scouterInitials = document.getElementById("input_s").value;
  var teamLabel = document.getElementById("teamname-label");
  
  // Update scouter name in all reminders
  var scouterSpans = document.querySelectorAll("#currentScouter");
  scouterSpans.forEach(span => {
    span.textContent = scouterInitials;
  });

  if (newNumber != "") {
    var teamName = getTeamName(newNumber);
    var displayText = teamName != "" ? newNumber + " - " + teamName : newNumber;
    teamLabel.innerText = teamName != "" ? "You are scouting " + teamName : "That team isn't playing this match, please double check to verify correct number";
    
    // Update all currentTeam spans
    var teamSpans = document.querySelectorAll("#currentTeam");
    ...
    var teamSpans = document.querySelectorAll("#currentTeam");
    teamSpans.forEach(span => {
      span.textContent = "NO TEAM SELECTED";
    });
  }
}
...
function counter(element, step) {
  ...
  if (result >= 0 || ctr.hasAttribute('data-negative')) {
    ctr.value = result;
    vibrate(VIBRATION_DURATION); // Vibrate on successful counter change
  } else {
    ctr.value = 0;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  // Set initial robot selection to B3
  document.querySelector('input[value="b3"]').checked = true;
