...
function resetRobot() {
  document.querySelector('input[value="b2"]').checked = true;
}
...
function updateMatchStart(event) {
  ...
  // Auto-fill team number for B2
  if (event.target.id == "input_m") {
    const match = getCurrentMatch();
    if (match && match.blue && match.blue.team_keys) {
      const b2TeamNumber = match.blue.team_keys[1].replace("frc", "");
      document.getElementById("input_t").value = b2TeamNumber;
      onTeamnameChange();
    }
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  // Set initial robot selection to B2
  document.querySelector('input[value="b2"]').checked = true;
